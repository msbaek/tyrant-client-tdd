...
이 프로젝트는 tokyo-tyrant의 java client를 만드는 시작 부분을 TDD로 진행하는
과정을 보여줍니다

Kent Beck의
[원본 video](http://pragprog.com/screencasts/v-kbtdd/test-driven-development)를
저자의 양해를 구해서 약간의 각색, 부연 설명을 추가했습니다

이 예제는 mac, [home brew](https://github.com/Homebrew/brew), IntelliJ 를
사용하는 것을 가정하고 진행됩니다.
...
Kent Beck은 TDD의 첫번째 단계는 해야 할 목록 즉 TODO 목록을 작성하는 것이라고
합니다.
...
원하는 것이 무엇인지를 표현하는 상위 레벨의 테스트로 시작한다. (이 상위 레벨
테스트를 Emily Bache는 목표 설계(Target Design)이라고도 부른다. 나는 첫번째
사용예를 통한 감 잡아보기라고도 부른다)
...
이걸 구현하려면 수십분이 걸리고, 테스트가 실패한다면 어디에 오류가 있는지 계속
따라 들어가면서 찾아야 한다.
...
모든 코드는 이야기 하듯이 작성되어야 한다(잘 쓰여진 산문처럼 읽을 수 있어야
한다).

그런데 이 코드는 테스트가 무엇을 하는지 이야기하고 있지 않고 어떻게
구현되었는지를 드러내고 있다(구현 디테일을 드러내고 있다).
...
TyrantMapFinalTest.java에 나머지 모든 테스트가 순차적으로 들어있고, 테스트를
추가하면서 구현한 기능이 있다.
